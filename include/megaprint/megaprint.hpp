...
[[nodiscard]] auto build_tree(T &&value, const inspect_method_options &options,
  ...
  if constexpr (simple_type<T>) {
    ...
    using possible_types = std::tuple<
        ...
        std::complex<float>, std::complex<double>, std::complex<long double>, std::exception,
        std::logic_error, std::domain_error, std::invalid_argument, std::length_error,
        std::runtime_error, std::range_error, std::overflow_error, std::underflow_error,
        std::bad_alloc, std::bad_cast, std::bad_typeid, std::bad_exception, std::system_error,
        std::ios_base::failure, std::filesystem::filesystem_error, std::exception_ptr,
        std::vector<bool>, std::bitset<4>, std::bitset<8>, std::bitset<16>, std::bitset<32>,
        std::bitset<64>, std::chrono::time_point<std::chrono::system_clock>,
