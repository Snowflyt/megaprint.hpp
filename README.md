...
- **Sequences:** `std::vector`, `std::list`, `std::forward_list` and `std::deque` are displayed as `type(size) [item1, item2, ...]`, e.g., `list(3) [1, 2, 3]`. For `std::vector`, `type(size)` is omitted, e.g., `[1, 2, 3]`. If the sequence is longer than the maximum length specified by `max_sequence_length`, it will be truncated and an ellipsis with the length of the truncation will be shown, e.g., `[1, 2, ... 3 more items]`.
