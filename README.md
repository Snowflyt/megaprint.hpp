...
| `styles`                   | `mp::styles`                                            | `{}`                                      | Custom styles for different types of values. Valid fields are `string`, `character`, `number`, `boolean`, `enumeration`, `null`, `time` and `special`. Valid colors are `bold`, `dim`, `reset`, `black`, `blue`, `cyan`, `gray`, `green`, `magenta`, `red`, `white`, and `yellow`. |
