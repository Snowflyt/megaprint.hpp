...
- **`std::atomic`:** `std::atomic` is treated transparently, similarly to `std::reference_wrapper`, e.g., `std::atomic<int> atomic_int{42};` is displayed as `42`, and `std::atomic<int *> atomic_ptr{&n};` is displayed as `&42`.
