...
- **All kinds of chars and strings:** signed and unsigned chars, `wchar_t`, `char8_t`, `char16_t`, `char32_t`, `std::string` and its variants (`std::wstring`, `std::u16string`, etc.), `std::string_view` and its variants, `std::pmr::string`, `const char[N]`, `const char *`, `const wchar_t *`, etc.
- **Pointers and Smart pointers:** `std::unique_ptr`, `std::shared_ptr`, `std::weak_ptr`, raw pointers, etc. Circular references are correctly displayed with a reference pointer (e.g., `<ref *1>`). Raw pointers are displayed with a `&` prefix, `std::unique_ptr` with a `~` prefix, `std::shared_ptr` with a `&[use_count]` prefix, and `std::weak_ptr` with a `&w[use_count]` prefix or `&w[expired]` if the pointer is expired.
...
| `styles`                   | `mp::styles`                                            | `{}`                                      | Custom styles for different types of values. Valid fields are `string`, `character`, `number`, `boolean`, `enumeration`, `null` and `special`. Valid colors are `bold`, `dim`, `reset`, `black`, `blue`, `cyan`, `gray`, `green`, `magenta`, `red`, `white`, and `yellow`. |
